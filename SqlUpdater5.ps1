function Install-SqlServerCumulativeUpdate
{
	...
	process {
		try
		{
			...
			$currentVersion = Get-SQLServerVersion -ComputerName $ComputerName -InstanceName $InstanceName
			...
			if ($PSCmdlet.ShouldProcess($ComputerName, "Install cumulative update [$($installer.Name)] for SQL Server [$($currentVersion.MajorVersion)]"))
			{
				...
				{
					write-host "How dare you! You want to restart the server!" ##Restart-Computer -ComputerName $ComputerName -Wait -For WinRm -Force
				}
...
function TestSqlServerServicePack
{
	...
	process {
		try
		{
			$currentVersion = Get-SQLServerVersion -ComputerName $ComputerName -InstanceName 'INSTSQL2016'
		...
		try
		{
			...
			else
			{
				...
				if ($PSCmdlet.ShouldProcess($ComputerName, "Install service pack [$($installer.Name)] for SQL Server [$($currentVersion.MajorVersion)]"))
				{
					...
					#######Write-Output "Output is here " + $installer.FullName + " Another path var " + $spExtractPath
					## Install the SP
					Invoke-Program @connParams -FilePath "$spExtractPath\setup.exe" -ArgumentList '/q /allinstances'
					
					if ($Restart.IsPresent)
					{
						write-host "How dare you! You want to restart the server!" ##Restart-Computer @connParams -Force -Wait
					}
				}
			}
		}
...
function Get-SQLServerVersion
{
	...
	process
	{
		try
		{
			$sqlInstance = Get-SQLInstance -ComputerName $ComputerName -Component SSDS -InsName $InstanceName
			if (-not $sqlInstance)
			{
...
function ConvertTo-VersionObject
{
	...
	process
	{
		try
		{
			...
			$filterScript = { $_.FullVersion -le "$($Version.Major).0.$($Version.Build).$($Version.Revision)" }
			...
			try
			{
				...
				if ($regKey.GetSubKeyNames() -contains "Instance Names")
				{
					foreach ($componentName in $Component)
					{
						...
						if ($regKey.GetValueNames() -eq $InsName)
						{
								Get-SQLInstanceDetail -RegPath $regPath -Reg $reg -RegKey $regKey -Instance $InsName;
				...
				{
					$isCluster = $false;
					$regKey.GetValue('InstalledInstances') 
					Get-SQLInstanceDetail -RegPath $regPath -Reg $reg -RegKey $regKey -Instance $InsName;
...
function Find-SqlServerServicePackInstaller
{
	...
	(
		...
		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$SqlServerInstallerBasePath = 'D:\Temp'
...
#>			
			...
			SQLServer2016-SP2-x64Write-Output "Output is here " + @icmParams + " Another path var " + $params
